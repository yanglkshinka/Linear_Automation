...
# # Stagger between SO batches (months)
# SO_STAGGER_MONTHS = 2

# # First SO base date (None = today UTC)
# BASE_DATE: Optional[datetime] = None

# # Lead (assignee)
# LEAD_ID = "70ef7c7b-49da-4cc8-8747-720b8394dbc6"

# # Your “golden” projects to copy issues/relations from (by name)
...
# # Per-SO Dynamics links (we’ll add these into a Resources issue)
...
# LINEAR_API_KEY = os.getenv("LINEAR_API_KEY")      # IMPORTANT: raw key; no "Bearer "
# LINEAR_TEAM_ID = os.getenv("LINEAR_TEAM_ID")      # schema expects teamIds: [String!]

# # Politeness
# SLEEP_BETWEEN_CALLS_SEC = 0.15
# DRY_RUN = False

# # Try to mirror template relations; fallback to default chain
...
#     try:
#         data = gql(q, {})
#         muts = data["__schema"]["mutationType"]["fields"]
#         return any(f["name"] == name for f in muts)
#     except Exception:
#         return False

# # ---------------- Project/Issue helpers ----------------
...
# def get_project_description(project_id: str) -> str:
#     q = """
#     query($id:String!){
...
#         return data["project"]["description"] or ""
#     except Exception:
#         return ""
...
#     """Load all issue labels into a name->id cache."""
#     after = None
#     while True:
#         q = """
#         query($first:Int!, $after:String){
...
#     inp = {
#         "name": name,
#         "description": description,   # keep short (<=255)
...
#     """
#     Create a project relation where predecessor (A) must finish before successor (B) can start.
#     Schema expects type='dependency' and lowercase anchors.
#     """
#     mutation = """
...
#         "type": "dependency",       # <-- your schema requires this
#         "anchorType": "end",
...
#     """
#     Read 'dependency' edges from source template projects.
#     Returns list of (phase_from, phase_to) as predecessor->successor.
#     """
...
#             pr = data["project"]["projectRelations"]
#             for n in pr["nodes"]:
#                 t = n["type"]
#                 a = n["project"]["name"]
#                 b = n["relatedProject"]["name"]
#                 if a in name_to_phase and b in name_to_phase:
#                     # Accept either "dependency" (new schema) or "blocks/blockedBy" (older)
#                     if t == "dependency":
...
# # ----- “Resources” issue + link/attachment (project-level external links not available) -----
...
#     if issue_id:
#         return issue_id
#     desc = "Project links and references:\n"
#     issue = create_issue(project_id, title, desc, [])
#     return issue["id"]

# def upsert_link_in_issue(issue_id: str, label: str, url: str):
#     # Append a bullet if not present
...
# def attach_link_if_supported(issue_id: str, label: str, url: str) -> bool:
...
#     # Try a rich attachment first; if not available, fall back to Markdown link
#     attached = attach_link_if_supported(issue_id, label, url)
#     if not attached:
...
#     # Warm label cache (so we can map/create labels quickly)
#     try:
#         warm_label_cache()
#     except Exception as e:
#         print(f"[WARN] Could not warm label cache: {e}")

#     # Pre-calc cumulative phase offsets (months)
...
#     # Cache template issues for each phase
...
#     # Try to inherit edges from template projects; fallback to default chain
#     blocks_edges: List[Tuple[str, str]] = []
#     if INHERIT_RELATIONS_FROM_TEMPLATES:
#         try:
#             blocks_edges = try_fetch_template_blocks_edges()
#         except Exception as e:
#             print(f"[INFO] Could not fetch template relations; falling back to default chain: {e}")

#     if not blocks_edges:
#         # Default adjacency: Sales->MP->Prod->QC->Ship
#         blocks_edges = [(PHASES[i], PHASES[i+1]) for i in range(len(PHASES)-1)]
#         print(f"[INFO] Using default relations chain: {blocks_edges}")
#     else:
#         print(f"[INFO] Inheriting template relations: {blocks_edges}")
...
#         # Create (or fetch) projects with sequential dates
...
#             target_date = iso_date(target_dt)

#             name = f"{so} {ph}"
#             desc = f"{so} – {ph}"  # keep short (<255)
...
#         # Clone issues (title + description + labels)
#         for ph in PHASES:
...
#         # Relations: predecessor -> successor (dependency end→start)
#         for a, b in blocks_edges:
...
#         # Add Dynamics link into a Resources issue per project
